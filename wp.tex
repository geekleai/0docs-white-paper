...
Contrary to the common perception that software development primarily involves writing new code, numerous studies have
 demonstrated that developers spend a substantial portion of their time navigating and understanding existing codebases.
  According to research by Ko et al. (2006), developers can spend up to 70\% of their time comprehending code
   when performing maintenance tasks [^1]. Similarly, Sillito et al. (2008) found that answering even simple questions
    about code often requires exploring multiple files and understanding the relationships between various code components [^2].

At Microsoft, internal studies have indicated that writing new code constitutes a relatively small fraction of a developer’s workflow. The majority of time is devoted to reading existing code, debugging, and comprehending how different parts of the system interact. This allocation of time highlights the importance of efficient code navigation tools and methodologies to enhance developer productivity and reduce time-to-market for software products.


\begin{description}
  \item[Initial Coin Offerings (ICOs)]
  Providing an efficient navigation framework without code editing capabilities.

  \item[Venture Capital Investment]
  Enabling agents to build a deep understanding of the code structure and functionality.

  \item[Airdrops]
  Facilitating the generation of meaningful documentation.
...
The document was originally created by Bohdan Snisar. 
